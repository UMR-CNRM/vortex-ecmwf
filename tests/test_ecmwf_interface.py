...
from vortex_ecmwf.tools.interfaces import ECMWFInterface

sh = ticket().sh


class TestPrepareArguments(TestCase):
    def _check_result_test(
        self,
        command,
        expected_command,
        list_args,
    ...
    def test_command_interface_true(self):
        interface = ECMWFInterface(
            system=sh, command="ecmwf", command_interface=True
        )
        # Test 1
        test_command_line = "ecfs.py ecp -i toto.txt titi.txt -value=toto,tata -m -target=toto2.txt".split()
        command, list_args, dict_args, list_options = (
            interface.prepare_arguments(test_command_line)
        )
        self._check_result_test(
            command,
            "ecp",
            list_args,
            ["toto.txt", "titi.txt"],
            dict_args,
            {"value": ["toto", "tata"], "target": "toto2.txt"},
            list_options,
            ["i", "m"],
        )
        # Test 2
    ...
    def test_command_interface_false(self):
        interface = ECMWFInterface(
            system=sh, command="ecmwf", command_interface=False
        )
        # Test 1
        test_command_line = "ecfs.py ecp -i toto.txt titi.txt -value=toto,tata -m -target=toto2.txt".split()
        command, list_args, dict_args, list_options = (
...
class TestBuildCommandLine(TestCase):
    def test_build_commandline(self):
        ...
        list_options = list()
        command_line = interface.build_command_line(
            command, list_args, dict_args, list_options
        )
        ...
        self.assertEqual(
            command_line,
            "ecmwf -value titi1 titi2 -u -verbose -r toto.txt titi.txt",
        )
